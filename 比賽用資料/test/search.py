...
def search(name):
    """搜尋賣香蕉的朋友"""
    global not_dealer                               #儲存以搜尋的名單
    dealer=deque()
    dealer+=graph[name]                             #搜尋串列先儲存Tom的朋友
    while dealer:
        person=dealer.popleft()                     #從左邊取資料
        if banana_dealer(person):                   #True表示找到了
            print(person+"是香蕉經銷商")
...
graph["Kevin"]=["Mary"]                             #建立字典graph, key="Kevin"的值
graph["Kevin"]=[]                                   #沒有其他朋友的空集合
graph["Peter"]=[]                                   #沒有其他朋友的空集合
graph["Banana"]=[]                                  #沒有其他朋友的空集合
graph["Mary"]=[]
